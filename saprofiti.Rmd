...
Ovdje uočavamo da je udio srčanih bolesnika, bolesnika s anginom i bolesnika s visokim sećerom raste svakom dobnom kategorijom, kao i vrijednosti OldPeak, dok MaxHR i Cholesterol padaju. To bi moglo potaknuti pitanja o ovisnosti dobi i vrijednosti tih parametara.
...
Vidimo da se iz grafova najviše isplati postaviti pitanje o ovisnosti MaxHR o dobi (iako je ta korelacija također dosta blaga), jer ostale ovisnosti iz grafova izgledaju još više nekorelirano.
...
```
S ovog boxplota vidimo se količine kolesterola više rasipaju u bolesnika te je desno zakrivljena, ali u pacijenata koji nisu srčani bolesnici ima mnogo stršećih vrijednosti pa je dobro promatrati medijane umjesto srednjih vrijednosti za ovaj primjer.
...
```
Slično prošlom primjeri, ovaj boxplot mogao sugerirati na razliku u vrijednosti Oldpeak parametra ovisno o tome je li pacijent srčani bolesnik ili ne - vidimo da za srčane bolesnike vrijedi da su vrijednosti Oldpeaka-a više te da medijan bude veći te čak izvan vrijednosti trećeg kvartila u skupini srčanih nebolesnika. Slično kao za kolesterol, za srčane bolesnike se vrijednost Oldpeaka manje rasipa (bude konzistentnije), ali ponovno imamo mnogo stršećih (atipičnih vrijednosti), zato je dobro promatrati medijane umjesto srednjih vrijednosti za ovaj primjer.


```{r}
boxplot(Age ~ HeartDisease, data = data)
```
Ponovno, ovaj boxplot mogao sugerirati na razliku u dobi pacijenata srčanih bolesnika i nebolesnika - vidimo da za srčane bolesnike vrijedi da su veće dobi te da medijan bude veći te čak izvan vrijednosti trećeg kvartila u skupini srčanih nebolesnika. 
