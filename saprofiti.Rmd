...
Prvo ćemo izvući osnovne mjere centralne tendencije i rasipanja za podatke kao što su minimum, maksimum, kvantili i srednja vrijednost za numeričke podatke i učestalost pojavljivanja po kategorijama za kategoričke. 
...
corrMat
```
Koeficijent korelacije je realan broj između -1 i 1, gdje veća apsolutna vrijednost prikazuje veću linearnu zavisnost. Iz tablice vidimo da varijable nisu toliko korelirane.
...
```
Iz rezultata testa možemo vidjeti da je p vrijednost manja od alfa = 5%. Test nam ukazuje da postoje statistički značajne razlike između podataka i normalne razdiobe. No kako nam prethodni grafovi pokazuju da su podatci približno normalno raspoređeni i kako je T test dosta robustan na normalnost podataka, možemo pretpostaviti da su podatci uzorkovani iz normalne razdiobe. I nastaviti s T testom no ipak treba imati na umu da rezultati mogu biti malo iskrivljeni.
...
Prilikom analize podatak, primjetili smo da 
...
```
Vidimo kako se podatci ne ravnaju savršeno po normalnoj razdiobi na repovima, koristeći logratiamsku funkciju za transformaciju ipak dobivamo bolje poklapanje s normalnom. Sličnost je dovoljno velika da bi mogli nastaviti s provođenjem testa (mala odstupanja od normalne razdiobe neće utjecati na rezultate zato što je T-test robusan na normalnost podataka).
...
```
Iz rezultata vidimo kako je p-vrijednost značajno manja od alfa = 5% pa zaključujemo da postoji razlika u razinama kolesterola između muškaraca i žena.


## 5. pitanje
## Postoji li korelacija ili homogenost između spola i pojave srčanih bolesti?

Kako bismo provjerili postoji li razlika u udjelu srčanih bolesnika među muškarcima i ženama, radimo test homogenosti.

H0: spoj i pojava srčanih bolesti su nezavisni
H1: postoji zavisnost spoja i pojave srčanih bolesti
...
```
Računamo opažene i očekivane frekvencije te na njima provodimo Chi-kvadrat test.
```{r}
...
Vidimo da je p-vrijednost manja od alfa = 5% što znači da odbacujemo hipotezu H0 u korist H1 koja tvrdi da postoji zavisnost između pojave srčanih bolesti i spola.